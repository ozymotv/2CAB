...
class Capture(threading.Thread):
    ...
    def setup_bettercam(self):
        try:
            self.bc = bettercam.create(
                device_idx=self.settings.get_int('Capture', 'bettercam_monitor_id'),
    ...
    def setup_obs(self):
        try:
            ...
            logging.error(f"Failed to setup OBS: {e}")
    ...
    def calculate_screen_offset(self, custom_region=[], x_offset=0, y_offset=0):
    ...
    def print_startup_messages(self):
        ...
        logging.info(f'[{self.settings.get("Hotkeys", "hotkey_targeting")}] - Aiming at the target')
        logging.info(f'[{self.settings.get("Hotkeys", "hotkey_exit")}] - EXIT')
        logging.info(f'[{self.settings.get("Hotkeys", "hotkey_pause")}] - PAUSE AIM')
        logging.info(f'[{self.settings.get("Hotkeys", "hotkey_reload_config")}] - Reload config')
